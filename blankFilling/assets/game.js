...
export default class InitGame {
  constructor({ container, configData, callback = () => {} }) {
    ...
    this.answer = this.shuffle(configData.answer);
    this.similar = this.shuffle(configData.similar);
  ...
  initTitle() {
    ...
    if (this.title.length > 8) {
      fontSize = Math.floor(420 / this.title.length);
    }
  ...
  initQuestion() {
    ...
    this.word.forEach((item, index) => {
      ...
      const allWidth =
        wordCount * fontSize + item.filter((i) => !i).length * 300;
      ...
      item.map((v, i) => {
        if (v) {
          ...
          const line = new Hilo.Graphics({ y: y + fontSize, x })
            .drawRect(0, 0, 300, 2)
            .beginFill()
            .endFill();
          x += 300;
    ...
    this.lineXY = lineArr.map((item) => ({
      x: item.x + container.x,
      y: item.y + container.y - 90,
    }));
  ...
  initOption() {
    ...
    const optionList = (this.optionList = this.option.map((item, index) => {
      ...
      const blueBg = this.getSourceBitmap("blueBg", {
        id: "blue",
        visible: false,
      });
      const yes = this.getSourceBitmap("yes", {
        id: "yes",
        x: 270,
        y: 50,
        visible: false,
      });
      const no = this.getSourceBitmap("no", {
        id: "no",
        x: 270,
        y: 50,
        visible: false,
      ...
      let fontSize = 40;
      if (item.length > 6) {
        fontSize = Math.floor(300 / item.length);
      }
      const text = new GameText({
        id: "text",
        text: item,
    ...
    optionList.forEach((optionContainer, index) => {
      ...
      optionContainer.on(Hilo.event.POINTER_END, (e) => {
        if (optionContainer.getChildById("blue").visible) {
          return false;
        }
        ...
        const result = this.lineXY.some((item, i) => {
          ...
          if (rule) {
            if (!this.myAnswer[i] || this.myAnswer[i] === text) {
              ...
              Hilo.Tween.to(
                optionContainer,
                { x: item.x, y: item.y },
                { duration: 100, ease: Hilo.Ease.Quad.EaseIn }
              );
            } else {
              const lastOption = optionList.filter(
                (oValue, oIndex) =>
                  oValue.getChildById("text").text === this.myAnswer[i]
              )[0];
              this.myAnswer[i] = text;
              Hilo.Tween.to(
                optionContainer,
              ...
              Hilo.Tween.to(
                lastOption,
                { x: lastOption.originX, y: lastOption.originY },
                { alpha: 1, duration: 100, ease: Hilo.Ease.Quad.EaseIn }
              );
          ...
          optionContainer.answerResult =
            this.myAnswer[i] === this.answer[i] ? "yes" : "no";
        ...
        if (!result) {
          ...
          Hilo.Tween.to(
            optionContainer,
            { x: optionContainer.originX, y: optionContainer.originY },
            { duration: 100, ease: Hilo.Ease.Quad.EaseIn }
          );
        }
        ...
        // 答完题显示提交
        if (
          this.myAnswer.every((answerVal) => answerVal) &&
  ...
  initResultButton() {
    const restart = this.getSourceBitmap("restart", { x: 1056, y: 960 }).addTo(
      this.stage
    );
    const result = this.getSourceBitmap("result", { x: 528, y: 960 }).addTo(
      this.stage
    );
    ...
    result.on(Hilo.event.POINTER_END, (e) => {
      this.optionList.forEach((item, index) => {
        if (item.answerResult === "no") {
          ...
          const changeOption = this.optionList.filter(
            (option, i) => rightText === option.getChildById("text").text
          )[0];
  ...
  getSourceBitmap(id, properties) {
    return new Hilo.Bitmap({
      image: this.quene.get(id).content,
      rect: this.quene.get(id).rect,
      ...properties,
    });
  }
}
