...
export default class ResultPanel extends Hilo.Container {
  ...
  creatContainer() {
    this.temporaryLinesContainer = new Hilo.Container({
      x: (1920 - 499 * 2 - 300) / 2,
      y: this.isText
        ? 320 - this.questionsLength * 10
        : 280 - this.questionsLength * 10,
    }).addTo(this)
  ...
  commonLine(left, right, image, isTween, type = 'panel', isError = false) {
    ...
    const rectLine = Math.floor(
      Math.sqrt(
        Math.pow(Math.abs(left - right), 2) + Math.pow(this.lineBase, 2)
      )
    )
    ...
    if (type === 'result') {
      const errorIcon = new Hilo.Bitmap({
        x: rectLine / 2 - 34,
        y: -20,
        ...
        visible: true,
        alpha: isError ? 1 : 0,
        rotation: -rotateContainer.rotation,
      }).addTo(rotateContainer)
    }
  ...
  line(properties) {
    ...
    if (this.repeatAnswerIndex.length) {
      this.repeatAnswerIndex.forEach((item, index) => {
        this.temporaryLinesContainer.removeChildById(
          this.setAnswer[item - index][0]
        )
    ...
    this.commonLine(
      basedistanceLeft,
      basedistanceRight,
      properties.images.rightLine,
      true
    )

    // 存储数据
    this.setAnswer.push([this.selected[0].realId, this.selected[1].realId])
    this.setAnswerQuestionsId.push([
      this.selected[0].questionId,
      this.selected[1].questionId,
    ])
    this.selected = []
  ...
  resutlLine(properties) {
    this.resultIds.forEach((item, index) => {
      let isError =
        properties.answerQuestionsIds[index][0] !==
        properties.answerQuestionsIds[index][1]
      ...
      if (properties.type === 'result') {
        images = isError
          ? properties.images.errorLine
          : properties.images.rightLine

        basedistanceLeft =
          properties.answerRealIds[index][0] * this.distance + 50

        basedistanceRight =
          properties.answerRealIds[index][1] * this.distance + 50
      } else {
        isError = !properties.answerRealIds.filter(
          (o) => o.join('') === item.join('')
        ).length

        images = isError
          ? properties.images.tipsLine
          : properties.images.rightLine
        ...
        img.onload = (e) => {
          ...
          const scale =
            realImageWidth > realImageHeight
              ? ((rect[2] - 40) * baseScale) / realImageWidth
              : ((rect[3] - 35) * baseScale) / realImageHeight
          new Hilo.Bitmap({
            id: { realId: index, questionId: item.id },
            x:
              realImageWidth > realImageHeight
                ? 0
                : (rect[2] - 80 - realImageWidth * scale) / 2,
            y:
              realImageWidth > realImageHeight
                ? (rect[3] - 60 - realImageHeight * scale) / 2
                : 0,
  ...
  panelClick(
    ...
    eventTarget.on(Hilo.event.POINTER_START, (e) => {
      ...
      selectedCanvas
        .lineStyle(4, '#ff2a2a')
        .drawRoundRect(0, 0, rect[2] * baseScale, rect[3] * baseScale, 20)
        .endFill()
  ...
  verifyRepeat() {
    // 验证是否选中两个
    if (!(this.selected.length === 2 && this.selected.every((item) => item)))
      return false

    this.setAnswer.forEach((item, index) => {
      if (
        item[0] === this.selected[0].realId ||
        item[1] === this.selected[1].realId
      ) {
